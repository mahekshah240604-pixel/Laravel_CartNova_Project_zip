...
<div class="layout">
    <nav class="sidebar">
        ...
        <a href="{{ route('admin.orders.index') }}" ><span class="icon">🛒</span> All Orders</a>
         <a href="{{ route('admin.returns') }}" class="active">🔁 Returns</a>
    ...
    <main class="main">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>#{{ $return->order->order_number ?? $return->order_id }}</td>
                    <td>{{ $return->user->name ?? $return->user_id }}</td>
                    <td>{{ $return->reason }}</td>
                    <td>{{ $return->message ?? 'No message' }}</td>

                    <td>
                        <span class="badge {{ $return->status }}">
                            {{ ucfirst($return->status) }}
                        </span>
                    </td>
                    <td>{{ $return->pickup_address }}</td>
                        <td>{{ $return->pickup_date ?? '-' }}</td>
                        <td>{{ ucfirst($return->pickup_status) }}</td>
                    <td>
                        <form method="POST" action="{{ route('admin.return.update', $return->id) }}">
                            @csrf
