...
<div class="layout">
    <nav class="sidebar">
        ...
        <a href="<?php echo e(route('admin.orders.index')); ?>" class="active"><span class="icon">🛒</span> All Orders</a>
    ...
    <main class="main">
        ...
        <div class="toolbar">
            <h1>🛒 Orders (<?php echo e($orders->total()); ?>)</h1>
            <form method="GET" style="display:flex;gap:8px;flex-wrap:wrap">
                <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Order # or customer name...">
                <select name="status" onchange="this.form.submit()">
                    <option value="all">All Status</option>
                    <?php $__currentLoopData = ['pending','confirmed','shipped','delivered','cancelled']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($s); ?>" <?php echo e(request('status')==$s?'selected':''); ?>><?php echo e(ucfirst($s)); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
                <button type="submit" class="btn btn-primary">Search</button>
                <?php if(request('search')||request('status')): ?>
                    <a href="<?php echo e(route('admin.orders.index')); ?>" class="btn" style="border-color:var(--border)">Clear</a>
                <?php endif; ?>
            </form>
        </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td style="font-size:.8rem;font-weight:700"><?php echo e($order->order_number); ?></td>
                    <td>
                        <div style="font-weight:700"><?php echo e($order->user->name ?? 'Deleted User'); ?></div>
                        <div style="font-size:.75rem;color:var(--muted)"><?php echo e($order->user->email ?? ''); ?></div>
                    </td>
                    <td style="white-space:nowrap;font-size:.82rem"><?php echo e($order->placed_at->format('d M Y')); ?></td>
                    <td><?php echo e($order->items->count()); ?> item(s)</td>
                    <td style="font-weight:700;white-space:nowrap">₹<?php echo e(number_format($order->total,0)); ?></td>
                    <td><span style="font-size:.78rem;background:#f0f0f0;padding:2px 6px;border-radius:3px"><?php echo e(strtoupper($order->payment_method)); ?></span></td>
    ...
    <form action="<?php echo e(route('admin.orders.destroy', $order->id)); ?>"
        ...
        <div class="pagination">
            <?php if($orders->onFirstPage()): ?><span class="disabled">← Prev</span><?php else: ?><a href="<?php echo e($orders->previousPageUrl()); ?>">← Prev</a><?php endif; ?>
            <?php $__currentLoopData = range(1,$orders->lastPage()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php if($p==$orders->currentPage()): ?><span class="active"><?php echo e($p); ?></span>
                <?php else: ?><a href="<?php echo e($orders->url($p)); ?>"><?php echo e($p); ?></a><?php endif; ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            <?php if($orders->hasMorePages()): ?><a href="<?php echo e($orders->nextPageUrl()); ?>">Next →</a><?php else: ?><span class="disabled">Next →</span><?php endif; ?>
        </div>
    </main>
</div>
<footer>© 1996–2025, CartNova · Admin Panel · All rights reserved</footer>
</body>
</html><?php /**PATH C:\xampp\htdocs\laravel\userside_project\resources\views/admin/orders/index.blade.php ENDPATH**/ ?>